...
class Solver:
    def __init__(self, banana_list):
        ...
        self.will_loop_map = self.get_will_loop_map(banana_list)

    def solve(self):
        pair_count_maps = list(filter(lambda m:m['count'] > 0, self.get_pair_count_maps()))
        pair_ct = 0
        
        while len(pair_count_maps) > 0:
            self.sort_pair_count_maps(pair_count_maps)
            
            target_index = pair_count_maps[0]['i']
            
            for pair_count_map in pair_count_maps:
                pair_index = pair_count_map['i']
                if self.will_loop_map[target_index][pair_index]:
                    pair_ct += 1
                    
                    for _map in pair_count_maps:
                        if self.will_loop_map[_map['i']][target_index]:
                            _map['count'] -= 1

                        if self.will_loop_map[_map['i']][pair_index]:
                            _map['count'] -= 1

                    pair_count_maps = list(filter(lambda m:m['i'] not in [pair_index, target_index] and m['count'] > 0, pair_count_maps))
                    break
        
        return len(self.banana_list) - 2 * pair_ct
    
    def get_will_loop_map(self, banana_list):
        size = len(banana_list)
        will_loop_map = [
            [False for  _i in range(0, size)]
            for _j in range(0, size)
        ]
        
        for i in range(0, size):
            for j in range(i + 1, size):
                if self.will_loop(banana_list[i], banana_list[j]):
                    will_loop_map[i][j] = True
                    will_loop_map[j][i] = True
        
        return will_loop_map
    
    def get_pair_count_maps(self):
        return [
            { 'i': i, 'count': sum(1 for j in self.will_loop_map[i] if j is True) }
            for i in range(0, len(self.banana_list))
        ]
    
    def sort_pair_count_maps(self, pair_count_maps):
        pair_count_maps.sort(key=lambda pair_count_map: pair_count_map['count'])
    
    # False when a + b can be described as 2^n * p (n, p >= 1) and a or b has p as a factor
    def will_loop(self, a, b):
        if a == b:
            return False
        ...
        # calc remain, s.t. a + b == 2^two_factor_ct * remain
        remain = a + b
        while remain % 2 == 0:
            remain /= 2

        return a % remain != 0 and b % remain != 0
    
def answer(banana_list):
    return Solver(banana_list).solve()
